...
function uploadfile($fieldName,$overwrite=false)
   {
	   ...
	   $uid=uniqid(); /* id unique pour image pour pouvoir utiliser des doublons*/
...
function securisation($donnees)
	{
                $donnees=htmlspecialchars($donnees); //code html injecter n'est pas interpreter s'affiche tel quel
                $donnees=trim($donnees);// efface les espacement rendu visible dans le code source et s'affiche normalement
                $donnees=stripslashes($donnees);//evite le slash
                $donnees=strip_tags($donnees);//code html injecter n'est pas interpreté et supprimé du rendu final
